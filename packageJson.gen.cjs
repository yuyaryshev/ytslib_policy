...
//        "precompile": "yb precompileinprint && ycplmon2 fix src && eslint src --fix --quiet",
        ...
        "watch:cjs": `npm run clean:cjs && title ${packageName} - watch:cjs & babel src --config-file ./babel.cjs.config.cjs --out-dir lib/cjs --extensions \".ts,.tsx,.js,.jsx\" --source-maps -w`,
        "watch:test:cjs": `${testModuleImports?`npm run test_module && `:""}${true?`npm run build:cjs && `:""}jest --config=jest.config-cjs.cjs --passWithNoTests --runInBand -w`,
        ...
        "watch:types": `npm run clean:types && title ${packageName} - watch:types & tsc -p tsconfig-declarations.json -w`,
        ...
        "test:cjs_jest": `${testModuleImports?`npm run test_module && `:""}${true?`npm run build:cjs && `:""}jest --config=jest.config-cjs.cjs --passWithNoTests --runInBand`,
        "test:cjs_mocha": `${testModuleImports?`npm run test_module && `:""}${true?`npm run build:cjs && `:""}mocha './lib/cjs/**/*.test.js'`,
        ...
        "test": `${testModuleImports?`npm run test_module && `:""}jest --config=jest.config-ts.cjs --passWithNoTests --runInBand`,
        ...(react?{
        ...
        ...(frontend?{
            ...
            "watch:frontend": `npm run precompile && title ${packageName} - watch:frontend & npm run clean:frontend && webpack-cli --mode development --config webpack.frontend.config.cjs -w`,
            "dev_server": `npm run precompile && title ${packageName} - dev_server & webpack-cli serve --mode development --config webpack.frontend.config.cjs`,
        }:{
